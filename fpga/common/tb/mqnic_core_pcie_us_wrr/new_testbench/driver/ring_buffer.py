...
class BufferDetails() :
    ...
    def __init__(self, num_tx_rings : int, num_rx_rings : int, num_eq_rings : int,
        driver_to_buffer : communication.CommunicationQueues,
        hardware_to_buffer : communication.CommunicationQueues) :
        
        self.num_tx_rings = num_tx_rings
        self.num_rx_rings = num_rx_rings
        self.num_eq_rings = num_eq_rings
        self.driver_to_buffer = driver_to_buffer
        self.hardware_to_buffer = hardware_to_buffer

def intialize_all_queues(num_tx_rings : int, num_rx_rings : int, num_eq_rings : int) :
    
    for i in range(0, num_tx_rings) :
        pass
    for i in range(0, num_rx_rings) :
        pass
    for i in range(0, num_eq_rings) :
        pass
    
    
async def buffer_coroutine(buffer_details : BufferDetails) :
       
    async def buffer_driver_coroutine() :
        while True: 
            memory_op : communication.MemoryOperation = await buffer_details.driver_to_buffer.receive_message()
    
    async def buffer_hardware_coroutine() :
        while True :
            memory_op : communication.MemoryOperation = await buffer_details.hardware_to_buffer.receive_message()
    
    # intialize_all_queues(buffer_details.num_tx_rings, buffer_details.num_rx_rings, buffer_details.num_eq_rings)

    driver_task = asyncio.create_task(buffer_driver_coroutine())
    hardware_task = asyncio.create_task(buffer_hardware_coroutine())
    await asyncio.gather(driver_task, hardware_task)
