		...
		struct mqnic_cq *cq, int size, int desc_block_size)
{
	...
	iowrite32(MQNIC_QUEUE_CMD_SET_SIZE | ilog2(ring->size) | (ring->log_desc_block_size << 8),
			ring->hw_addr + MQNIC_QUEUE_CTRL_STATUS_REG);
	// set function id
	iowrite32(MQNIC_QUEUE_CMD_SET_VF_ID | 0,
			ring->hw_addr + MQNIC_QUEUE_CTRL_STATUS_REG);

	printk(KERN_INFO "Scott TX VF ID %d", ioread32(ring->hw_addr + MQNIC_QUEUE_BASE_ADDR_VF_REG) & 0x00ff);	
	// set CQN
	cqn = ring->cq->cqn;
	iowrite32(MQNIC_QUEUE_CMD_SET_CQN | cqn,
			ring->hw_addr + MQNIC_QUEUE_CTRL_STATUS_REG);
	// set pointers
